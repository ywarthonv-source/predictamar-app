...
def score_a_indice(score, ok):
    """Calcula el indice operativo — no es probabilidad estadistica."""
...
def indice_a_semaforo(score):
    """Devuelve color y etiqueta del semaforo segun MacroScore."""
    if score >= 0.65:
        return "🟢", "VERDE", "#1B5E20"
    elif score >= 0.55:
...
def generar_imagen_bytes(idx, row, species, modo, fase_emoji, fase_nombre):
    ...
    sem_emoji, sem_label, sem_color = indice_a_semaforo(row.get("score", 0))
    ...
    # Header
    ax.add_patch(FancyBboxPatch((0, 15.5), 10, 1.5,
                  boxstyle="round,pad=0.1", facecolor=ch, edgecolor="none"))
    ax.text(5, 16.55, "PREDICTAMAR  " + em + "  " + sp,
            ha="center", va="center", fontsize=13,
            fontweight="bold", color="white", fontfamily="monospace")
    ax.text(5, 16.05, "Zona: " + modo,
            ha="center", va="center", fontsize=8, color="white")

    # Semaforo
    ax.add_patch(FancyBboxPatch((0.3, 14.7), 9.4, 0.7,
                  boxstyle="round,pad=0.08",
                  facecolor=sem_color, alpha=0.15,
                  edgecolor=sem_color, linewidth=1.5))
    ax.text(5, 15.05, sem_emoji + "  " + sem_label,
            ha="center", va="center", fontsize=11,
            fontweight="bold", color=sem_color)

    # Número
    ax.add_patch(plt.Circle((5, 14.1), 0.55, color=ch, zorder=3))
    ax.text(5, 14.1, str(idx + 1),
    ...
    ax.text(5, 13.3, "Indice operativo: " + str(indice) + "%",
    ...
    ax.plot([0.5, 9.5], [12.55, 12.55], color=ch, linewidth=1.5, alpha=0.4)

    # Coordenadas
    ax.text(0.5, 12.2, "📍", fontsize=11, va="center")
    ax.text(1.3, 12.2,
...
if buscar:
    ...
    # MacroScore promedio de los resultados
    macro_score_zona = float(res["score"].mean())
    sem_emoji, sem_label, sem_color = indice_a_semaforo(macro_score_zona)
    ...
    st.markdown(
        ...
        {sem_emoji} ZONA {sem_label}
        </span>
        <span style="font-size:0.9em; color:#555; margin-left:12px;">
        MacroScore: {macro_score_zona:.2f}
        </span>
        </div>
        """,
